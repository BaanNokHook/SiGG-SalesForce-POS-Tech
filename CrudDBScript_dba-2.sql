...
CREATE DEFINER=`root`@`localhost` PROCEDURE `bill` (IN `oid` INT) begin          
  ...
  DECLARE curl CURSOR FOR SELECT sum(Amount) as tot_amount FROM Order_items where Order_id=oid group by Order_id ;     
  ...
  set disc=(SELECT Discount from bill where Order_id=i=oid);                           
    update table_master set booking_stat="not_booked" where table_id=tid;     
    update table_master set Cust_id=null wherer table_id=tid;                    
  OPEN cur1;  
  SET b = 0;  
